...
// ១. ដាក់បញ្ចូល Telegram Bot Token និង Chat ID របស់អ្នកនៅទីនេះ
const TELEGRAM_BOT_TOKEN = '8777326333:AAFZg2atygjaoNYuzrFE-XR_YzjrMlbyJAs';
const TELEGRAM_CHAT_ID = '8589768049';

// ២. មុខងារសម្រាប់ទាញយកព័ត៌មាន Device របស់អ្នកប្រើប្រាស់
const getDeviceInfo = () => {
...
// ៣. មុខងារផ្ញើសារទៅកាន់ Telegram Bot
const sendDeviceToTelegram = async () => {
  // ការពារកុំឱ្យផ្ញើសារជាន់គ្នាច្រើនដងក្នុង Session តែមួយ (Optional)
  if (sessionStorage.getItem('device_sent')) return;

  const device = getDeviceInfo();
  
  const message = `
🔔 *មានអ្នកចូលមើល Website របស់អ្នក!*
📱 *Device Type:* ${device.deviceType}
💻 *Platform:* ${device.platform}
🖥️ *Screen Size:* ${device.screenResolution}
...
⏰ *Time:* ${new Date().toLocaleString()}
  ...
  try {
    ...
    // កត់ចំណាំថាបានផ្ញើរួចហើយដើម្បីកុំឱ្យផ្ញើ Repeat ពេល Refresh
    sessionStorage.setItem('device_sent', 'true');
  } catch (error) {
